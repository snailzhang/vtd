...
<body>
	...
	<div class="container">
		<div class="well well-sm">
			<span>FTP地址：</span> <span>${ftpUrl}</span>
		</div>
	</div>
	...
	<div class="container">
		...
		<ul class="nav nav-tabs" role="tablist">
			<li class="active"><a href="#packUncomplete" role="tab" data-toggle="tab">未完成任务包列表</a>
			</li>
			<li><a href="#packComplete" role="tab" data-toggle="tab">已完成任务包列表</a>
			</li>
			<li><a href="#packAll" role="tab" data-toggle="tab">全部任务包列表</a>
			</li>
			<li><a href="#publishList" role="tab" data-toggle="tab">待发布任务包列表</a>
			</li>
			<li><a href="#packUnzip" role="tab" data-toggle="tab">未上传任务包列表</a>
			</li>
			<li><a href="#editTaskRole" role="tab" data-toggle="tab">编辑标注说明</a>
			</li>
		</ul>
		<div class="tab-content">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<form role="form" class="form-inline">
							<div class="form-group" id="">
								<p class="form-control-static">包类型：</p>
							</div>
							<div class="form-group" id="">
								<select class="form-control" name="taskType" id="taskType">
									<option value="1">一层</option>
									<option selected="selected" value="2">三层</option>
							...
							<div class="form-group" id="">
								<p class="form-control-static">包等级：</p>
							</div>
							<div class="form-group" id="">
							...
							<div class="form-group" id="packLockTimeDiv">
								<div class="input-group">
									<input type="text" class="form-control" name="packLockTime" id="packLockTime" placeholder="添加任务时间" onkeydown="if(event.keyCode==13){return false;}"> <span class="input-group-addon">小时</span>
								</div>
							</div>
						</form>
					</div>
			...
			<div class="tab-pane" id="editTaskRole">
				<div class="panel panel-default">
					<div class="panel-heading">编辑任务说明</div>
					<div class="panel-body">
						<form role="form" class="form-horizontal">
							<div class="form-group" id="">
								<label for="noteTitle" class="col-sm-2 control-label">说明名称：</label>
								<div class="col-sm-6">
									<input type="text" class="form-control" name="noteTitle" id="noteTitle" placeholder="规则名称" required="required" autocomplete="off"> <span class="help-block"></span>
								</div>
							</div>
							<script type="text/plain" id="myEditor" style="width:960px;height:240px;"></script>
						</form>
						<div style="margin:10px;">
							<button type="button" id="saveEdit" class="btn btn-primary btn-lg center-block">增加新说明</button>
						</div>
					</div>
	...
	<!-- /.modal -->
	<!-------------------------------- 修改包类型方法-------------------------------------------------->
	<div id="changePackTypeModal" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">
					...
					<h4 class="modal-title" id="updatePackType-title"></h4>
				</div>
				<div class="modal-body">
					<span id="updateStatus" class="text-center"></span>
					<form class="form-horizontal" role="form">
						<div class="form-group" id="">
							<label for="ctLvl" class="col-sm-3 control-label">包类型：</label>
							<div class="col-sm-9" style="width: 200px;">
								<select class="form-control" name="packType-selected" id="packType-selected" style="width: 150px;">
									<option value="1">一层</option>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" id="changePackTypeBtn" class="btn btn-primary">修改</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<script type="text/javascript">
				...
				.ready(
						function() {
							...
							$("#un_searchBtn").click(
									function() {
										unpackNameCondition = $(
												"#unpackNameCondition").val();
										upzipCondition = $("#upzipCondition")
												.val();
							...
							$("#searchBtn").click(
									function() {
										packNameCondition = $(
												"#packNameCondition").val();
										loadCompletePackList(1);
									});
							$("#searchAllBtn").click(
									function() {
										allPackNameCondition = $(
												"#allPackNameCondition").val();
										loadAllPackList(1);
									});
							...
							$("#searchPublishPackBtn").click(
									function() {
										publishCondition = $(
												"#publishCondition").val();
										loadPublishList(1);
									});
							$("#changeTaskLvlBtn")
									.click(
							...
							$("#saveEdit")
									.click(
											function() {
												...
												hasContent = UM.getEditor(
														'myEditor')
														.hasContents();
												if (hasContent) {
													var con = UM.getEditor(
															'myEditor')
															.getContent();
													...
													$
															.ajax({
																...
																success : function(
																		data) {
																	if ($("#editTaskRole .panel-body .alert").length > 0)
																		$(
																				"#editTaskRole .panel-body .alert")
																				.remove();
																	var $alertMsg = $("<div class='alert alert-dismissable'>"
																			+ data.message
																			+ "</div>");
																	$alertMsg
			...
			$
					.ajax({
						...
						data : {
							"packStuts" : 0,
							"page" : pageNum,
							"packNameCondition" : unpackNameCondition,
							"unzip" : upzipCondition
						},
						dataType : 'json',
						success : function(data) {
							if (data.list == "") {
								$("#packUncomplete tbody").empty();
								$("#packUncomplete tbody")
										.append(
												"<tr class='text-danger'><td colspan='12'>无内容</td></tr>");
							} else {
								$("#packUncomplete tbody").empty();
								$
										.each(
												...
												function(i, item) {
													if (item.unzip == 0) {
														...
														var invalidCount = item.invalid
																+ item.wavZero;//无效数
														var surplusTask = item.taskCount
																- item.finishTaskCount
																- invalidCount;//未完成任务数

														var finishTaskRatio = item.finishTaskCount
																/ (item.taskCount - invalidCount)
																* 100;//完成任务比例
														finishTaskRatio = finishTaskRatio
																.toFixed(2);
														var downloadPack = "<td></td>";
														if (item.finishTaskCount != 0) {
															downloadPack = "<td><a href='#' id='dp"
																	+ item.packId
																	+ "' class='downloadPack' onClick='downloadPackFn("
																	+ item.packId
																	+ ")'>下载 <span class='badge'>"
																	+ item.downCount
																	+ "</span></a></td>";
															...
															downloadPack = "<td><a href='#' id='dp"
																	+ item.packId
																	+ "' class='downloadPack' onClick='downloadPackFn(0)'>下载 <span class='badge'>0</span></a></td>";
														}
														var pausePackBtn = "";
														if (item.unzip == 3) {
															pausePackBtn = "<td><a href='javascript:changePackStatusFn("
																	+ item.packId
																	+ ",1,1)'>启用</a></td>";
														} else {
															pausePackBtn = "<td><a href='javascript:changePackStatusFn("
														...
														$(
																...
																.append(
																		"<tr>"
																				...
																				+ (i + 1)
																				+ "</td>"
																				+ "<td><a href='#' class='packId' onClick='showPackDetail("
																				+ item.packId
																				+ ")'>"
																				+ item.packName
																				+ "</a></td>"
																				+ "<td><a href='javascript:changeTaskLvl("
																				+ item.packId
																				+ ","
																				+ item.taskLvl
																				+ ");'>"
								...
								page.creatPageHTML(pageNum, pucPageTotle,
										pageDom, "loadUnCompletePackList");
								$("#packUncomplete .pageGoBtn")
										.click(
												function() {
													var pageNum = 0;
													pageNum = $(
			...
			$
					.ajax({
						...
						data : {
							"packStuts" : 1,
							"page" : pageNum,
							"packNameCondition" : packNameCondition,
							"unzip" : 1
						},
						dataType : 'json',
						success : function(data) {
							if (data.list == "") {
								...
								$
										.each(
												...
												function(i, item) {
													...
													var downloadPack = "<td><a href='#' id='dp"
															+ item.packId
															+ "' class='downloadPack' onClick='downloadPackFn("
															+ item.packId
															+ ")'>下载<span class='badge'>"
								...
								page.creatPageHTML(pageNum, pcPageTotle,
										pageDom, "loadCompletePackList");
								$("#packComplete .pageGoBtn")
										.click(
												function() {
													var pageNum = 0;
													pageNum = $(
															"#packComplete .pageGoText")
															.val();
													if (pageNum != 0
															&& 0 < pageNum
															&& pageNum < pcPageTotle + 1) {
			...
			$
					.ajax({
						...
						data : {
							"packStuts" : 3,
							"page" : pageNum,
							"packNameCondition" : allPackNameCondition,
							"unzip" : 0
						},
						dataType : 'json',
						success : function(data) {
										...
										.each(
												...
												function(i, item) {
													...
													var surplusTask = item.taskCount
															- item.finishTaskCount
															- invalidCount;//未完成任务数
													var taskCountTotle = item.taskCount
															- item.invalid
															- item.wavZero;
													var finishTaskRatio = item.finishTaskCount
															/ taskCountTotle
		...
		loadPublishList = function(pageNum) {
			$.ajax({
				...
				data : {
					...
					"page" : pageNum,
					"unzip" : 2,
					"packNameCondition" : publishCondition
				},
				dataType : 'json',
				success : function(data) {
					if (data.list == "") {
						...
						$.each(data.list,function(i, item) {
							...
							$("#publishList tbody").append(
								...
								+ item.packName
								+ "</td>"
								+ "<td><a href='javascript:changeMarkTimeMethod("+ item.packId+ ",\""+ item.markTimeMethodName+ "\",\""+ item.packName+ "\");'>"
								+ item.taskMarkTimeName
								+ "</a></td>"
								+ "<td>"
								+ item.taskCount
								+ "</td>"
								...
								+ item.packLockTime
								+ "小时</td>"
								+ "<td><a href='javascript:changePackType("+ item.packId+ ",\""+ item.packType+ "\",\""+ item.packName+ "\");'>"
								+ item.packType
								+ "</a></td>"
								+ publishPackBtn
								+ deletePackBtn
						...
						pcPageTotle = data.totlePage;
						page.creatPageHTML(pageNum, pcPageTotle,
								pageDom, "loadPublishList");
						$("#publishList .pageGoBtn")
								.click(
										function() {
											var pageNum = 0;
											pageNum = $(
													"#publishList .pageGoText")
													.val();
											if (pageNum != 0
													&& 0 < pageNum
													&& pageNum < pcPageTotle + 1) {
					...
					.ajax({
						...
						success : function(data) {
							...
							if (data.markTimeMethodList == "") {
								$("#markTimeMethodList").html(
										"<option value='0'>无</option>");
							} else {
								$.each(data.markTimeMethodList, function(i,
										item) {
									$("#markTimeMethodList").append(
							...
							if (data.list != "") {
													...
													$("#packUnzip tbody")
															.append(
																	"<tr class='unziptr"+i+"'>"
																			...
																			+ item
																			+ "</td>"
																			+ "<td class='packZipStatus'><a href='javascript:zipOnePack(\""
																			+ item
																			+ "\",\"unziptr"
																			+ i
																			+ "\");'>上传</a></td>"
								...
								$("#packUnzip tbody")
										.append(
												"<tr><td colspan='3'>无未上传任务包</td></tr>");
							}
						}
					});
		};
		zipOnePack = function(pName, trClass) {
			var taskTypec = $("#taskType").val();
			var taskType = $("#taskType option[value="+taskTypec+"]").text();
			$("#taskTypeCon").text(taskType);
					...
					function() {
						...
						$.ajax({
							...
							data : {
								...
								"taskType" : taskTypec,
								"taskLvl" : taskLvl,
								"packLockTime" : packLockTimeObj.val(),
								"noteId" : noteId,
								"markTimeMethod" : markTimeMethod
							},
							dataType : 'json',
							success : function(data) {
								$("." + trClass + " .packZipStatus").text(
										data.message);
							}
						});
			...
			$
					.ajax({
						...
						data : {
							...
							"page" : pageNum,
							"taskStuts" : 2,
							"taskNameCondition" : taskNameCondition
						},
						dataType : 'json',
						success : function(data) {
							if (data.list == "") {
								...
								$
										.each(
												...
												function(i, item) {
													var te = "";
													if (item.taskEffective != null)
														te = item.taskEffective;
													$("#packDetailTBody")
															.append(
																	"<tr>"
																			+ "<td>"
																			+ (i + 1)
																			+ "</td>"
																			+ "<td><a href='javascript:selectTask("
																			+ item.taskId
																			+ ",\""
																			+ item.taskName
																			+ "\");'>"
								...
								page.creatPageHTML(pageNum, pdPageTotle,
										pageDom, "loadPackDetailList");
								$("#packDetailModal .pageGoBtn")
										.bind(
												"click",
												function() {
													var pageNum = 0;
													pageNum = $(
															"#packDetailModal .pageGoText")
															.val();
													if (pageNum != 0
															&& 0 < pageNum
															&& pageNum < pdPageTotle + 1) {
		...
		changeTaskLvl = function(packId, taskLvl) {
			...
			$("#ctLvl option[value=" + taskLvl + "]").attr("selected",
					"selected");
		...
		changeMarkTimeMethod = function(packId, markTimeMethodName, packName) {
			...
			$.ajax({
				...
				success : function(data) {
					if (data.list == "") {
						$("#markTimeMehtodName").append(
								"<option>无说明文件</option>");
					} else {
						$("#markTimeMehtodName-selected").empty();
						$.each(data.list,function(i, item) {
							$("#markTimeMehtodName-selected").append(
		...
		/*---------------------------------------修改包类型方法---------------------------------------------------------------*/
		changepackType = function(packId, packType, packName) {
			$("#updatePackType-title").text("修改" + packName + "类型");
			$("#updateStatus").empty();
			$("#changePackTypeModal").modal('show');
			packTypePackId = packId;
		};
		/*--------------------------------------修改包类型提交--------------------------------------------*/
		$("#changePackTypeBtn").click(function() {
			var packTypeId = $("#packType-selected").val();
			$.ajax({
				type : 'POST',
				data : {
					"packId" : packTypePackId,
					"packTypeId" : packTypeId
				},
				url : '${contextPath}/security/updatePackTypeId',
				dataType : 'json',
				success : function(data) {
					if (data.replay == 1) {
						alert("修改成功");
					...
					.ajax({
						...
						success : function(data) {
							if (data.list == "") {
								...
								$("#selectTaskTBody")
										.append(
												"<tr class='text-danger'><td colspan='8'>无内容</td></tr>");
							} else {
								$("#selectTaskTBody").empty();
								$.each(data.list, function(i, item) {
